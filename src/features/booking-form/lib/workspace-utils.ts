...
export function parseWorkspaceDates(
	...
	// Fallback: use expectedCompletionDate + durationMonths
	if (serviceItem.expectedCompletionDate) {
		const start = new Date(serviceItem.expectedCompletionDate);
		const months = serviceItem.durationMonths || 1;
		return { startDate: start, endDate: addDays(start, months * 30 - 1) };
	}
