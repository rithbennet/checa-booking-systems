...
export function AdminStatusChips({
	...
	const isPendingGroup =
		active?.length === 1 && active.includes("pending_approval");

	// Primary / uncommon flags
	const isRevisionRequested =
		active?.length === 1 && active[0] === "revision_requested";
	...
	return (
		<div className="flex flex-wrap items-center gap-2">
			...
			<Badge
				...
				variant="secondary"
			>
				Pending {counts ? `(${counts.pending_approval})` : ""}
			</Badge>
			...
			<Badge
				...
				variant="secondary"
			>
				{ADMIN_STATUS_LABELS.in_progress}{" "}
				{counts ? `(${counts.in_progress})` : ""}
			</Badge>
				...
				<PopoverTrigger asChild>
					<Badge
						className={`cursor-pointer ${
							isRevisionRequested
								? `${getChipClassName(true, "revision_requested")} ring-2 ${
										getAdminStatusColors("revision_requested").ring
									}`
								: isCompleted
									? `${getChipClassName(true, "completed")} ring-2 ${
											getAdminStatusColors("completed").ring
										}`
									: isRejected
										? `${getChipClassName(true, "rejected")} ring-2 ${
												getAdminStatusColors("rejected").ring
											}`
										: isCancelled
											? `${getChipClassName(true, "cancelled")} ring-2 ${
													getAdminStatusColors("cancelled").ring
												}`
					...
					<div className="space-y-1">
							...
							className={
								isRevisionRequested
									? `w-full justify-between font-medium ${
											getAdminStatusColors("revision_requested").bg
										} ${
						...
						<Button
							className={
								isCompleted
									? `w-full justify-between font-medium ${
											getAdminStatusColors("completed").bg
										} ${
						...
						<Button
							className={
								isRejected
									? `w-full justify-between font-medium ${
											getAdminStatusColors("rejected").bg
										} ${getAdminStatusColors("rejected").text} hover:opacity-95`
									: "w-full justify-between"
							}
						...
						<Button
							className={
								isCancelled
									? `w-full justify-between font-medium ${
											getAdminStatusColors("cancelled").bg
										} ${
