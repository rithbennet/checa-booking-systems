...
export function mapBookingToCreateBookingInput(booking: {
	...
	serviceItems?: Array<{
		...
		serviceAddOns?: Array<{ addOnCatalogId: string }>;
	}>;
	...
	const toAddOnIds = (
		addOns?: Array<{ addOnCatalogId: string } | null | undefined>,
	): string[] => {
		if (!Array.isArray(addOns)) {
			return [];
		}
		const ids = addOns
			.map((addOn) => addOn?.addOnCatalogId)
	...
	return {
		...
		serviceItems:
			booking.serviceItems?.map((item) => ({
				...
				// DB BookingServiceItem does not store durationMonths; default to 0
				durationMonths: 0,
				...
				otherEquipmentRequests: Array.isArray(item.otherEquipmentRequests)
					...
					: undefined,
				addOnCatalogIds: toAddOnIds(item.serviceAddOns),
			})) ?? [],
		workspaceBookings:
			booking.workspaceBookings?.map((workspace) => ({
				...
				addOnCatalogIds: toAddOnIds(workspace.serviceAddOns),
			})) ?? [],
